...
Test loss comparison (Table 1 in paper):
![Figure](new_figures/covtype16.png)

We supplement the baseline results of dataset Covtype as above. For N=16, RFLAF with RBF achieves the lowest test error, and RFLAF with BS is the second best. The standard RF models all have losses larger than 0.500. The results of $N=32,64$ are similar and can be found in the `Expe_Tables_in_paper.ipynb`.

Test time comparison (Table 2 in paper):
![Figure](new_figures/covtype16time.png)
